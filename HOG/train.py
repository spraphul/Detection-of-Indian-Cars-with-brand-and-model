...
image_path_logo = os.path.join(cwd,'logo.jpg')
image_path = os.path.join(cwd,'test.jpg')
train_path = os.path.join(cwd,'train_data')
save_path = os.path.join(cwd,'saved_weights')
...
def train_values():
	...
	with tf.Session() as sess:
		...
		write_ckpt(W,sess,'weights')
		write_ckpt(b,sess,'biases')

		weight = sess.run(W)
		bias = sess.run(b)

	test = generate_random_test(50)
	X,Y = create_labels(0, test, 50, 50)
	accu = evaluate_accuracy(classify(X, weight.astype(dtype=np.float32), bias.astype(dtype=np.float32)), Y)
	with tf.Session() as sess:
...
def write_ckpt(tensor, sess, name):
...
def train_SVM():
	...
	test = generate_random_test(50)
	X,Y = create_labels(0, test, 50, 50)
	accu = evaluate_accuracy(SVM_classify(X, W_main.astype(dtype=np.float32), b_main.astype(dtype=np.float32)), Y)
	with tf.Session() as sess:
		print('\nTest Accuracy : '+str(sess.run(accu))+' % ....')
	...
	with tf.Session() as sess:
		sess.run(tf.global_variables_initializer())
		write_ckpt(W_final,sess,'weights')
		write_ckpt(b_final,sess,'biases')
...
if __name__ == '__main__':
	...
	if sys.argv[1]=='train':
		...
		if line=='SVM':		
			...
			prediction = SVM_classify(X,W_tensor,b_tensor)
			...
			X = hog.hog_from_path(image_path)
			prediction = classify(X,W_tensor,b_tensor)
		...
		with tf.Session() as sess:
			read_ckpt(os.path.join(save_path,'weights/weights.ckpt'),'weights',W,sess)
			read_ckpt(os.path.join(save_path,'biases/biases.ckpt'),'biases',b,sess)
		...
		if line=='SVM':
			prediction = SVM_classify(X,W_array,b_array)
		elif line=='LOGIST':
			prediction = classify(X,W_array,b_array)
